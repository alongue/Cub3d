...
void	set_used_poly(t_polygon *set, t_polygon *current)
{
	...
	while (set[i].exist)
	{
		//if (set[i].isused)
			set[i].isused = 0;
		//vscode printf("set[%d].segment.exist = %d\t\tet\t\tcurrent->segent.exist = %d\n", i, set[i].segment.exist, current->segment.exist);
